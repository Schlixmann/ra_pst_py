...
def cp_solver(ra_pst_json, warm_start_json=None, log_file = "cpo_solver.log", timeout=100, break_symmetries:bool=False, sigma:int=10):
    """
...
def cp_solver_alternative(ra_pst_json, warm_start_json=None, log_file = "cpo_solver.log", replan = False, release_time:int=0):
    """
    ra_pst_json input format:
    {
        "resources": [resourceId],
...
def cp_solver_alternative_new(ra_pst_json, warm_start_json=None, log_file = "cpo_solver_new.log", timeout = 100, replan = False, release_time:int=0, break_symmetries:bool=False):
    """
    ra_pst_json input format:
    {
        "resources": [resourceId],
...
def cp_solver_alternative_online(ra_pst_json, warm_start_json=None, log_file = "cpo_solver_new.log", timeout = 100, replan = False, release_time:int=0, break_symmetries:bool=False, horizon:int= 20):
    """
    ra_pst_json input format:
    {
        "resources": [resourceId],
    ...
    for i, instance in enumerate(ra_psts["instances"]):
        ...
        for taskId, task in instance["tasks"].items():
            ...
            # jobs of this task:
            jobs_of_task = {jobId:job for jobId, job in instance["jobs"].items() if job["selected"]}
            # if the min(job["start"]) is smaller than releas time, task["fixed"] == true
            if len(jobs_of_task) > 0:                
                if min([job["start"] for job in jobs_of_task.values()]) < release_time:
                    task["fixed"] = True
                    # set job["fixed"] = True for all jobs of this task
                    for jobId, job in jobs_of_task.items():
                        job["fixed"] = True
    ...
    for ra_pst in ra_psts["instances"]:
        ...
        # Create optional interval variables for each job
        for jobId, job in ra_pst["jobs"].items():
            min_time = job["release_time"] if not job["start"] else job["start"]
            if min_time >= int(release_time) and job["release_time"] <= horizon: # If best case release time is in time_window
                job["interval"] = model.interval_var(name=jobId, optional=True, size=int(job["cost"]))
                job["interval"].set_start_min(int(min_time))
            elif job["release_time"] >= horizon:
                job["fixed"] = True
                continue
            else:
                job["interval"] = model.interval_var(name=jobId, optional=False, size=int(job["cost"])) #model.interval_var(name=jobId, optional=False, size=int(job["cost"]))
