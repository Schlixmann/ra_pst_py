...
def cp_solver_final(ra_pst_json, warm_start_json=None, log_file = "cpo_solver.log", timeout=100, break_symmetries:bool=False, sigma:int=0):
    """
    ra_pst_json input format:
    {
        "resources": [resourceId],
            ...
            for jobId, job in ra_pst["jobs"].items():
                if job["selected"]:
                    ...
                    if job["start"] is not None:
                        ...
                        job["interval"].set_start_max(start_hr + sigma) 
                        job["interval"].set_end_min(end_hr)
                        job["interval"].set_end_max(start_hr + sigma)
                    #job_intervals.append(job["interval"])
                    fixed_intervals += 1
