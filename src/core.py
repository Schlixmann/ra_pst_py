...
class TaskAllocation(RA_PST):
    ...
    def __init__(self, parent: RA_PST, task: str, state='initialized') -> None:
        ...
        self.task_elements = [f"{{{self.ns['cpee1']}}}manipulate", f"{{{self.ns['cpee1']}}}call"]
    ...
    def allocate_task(self, root=None, resource_url: etree = None, excluded=[]):
        ...
        if len(root.xpath("cpee1:children", namespaces=self.ns)) == 0:  # no task parents exist
            if len([parent for parent in root.xpath("ancestor::cpee1:*", namespaces=self.ns) if parent.tag in self.task_elements]) == 0:
                warnings.warn("No resource for a core task")
                raise (ResourceError(root))
            else:
                task_parents = [parent for parent in root.xpath(
                    "ancestor::cpee1:*", namespaces=self.ns) if parent.tag in self.task_elements]
                task_parent = task_parents[-1]

            if len(task_parent.xpath("cpee1:children/*", namespaces=self.ns)) == 0:
                warnings.warn(
                    "The task can not be allocated due to missing resource availability", ResourceWarning)
        ...
        for profile in root.xpath("cpee1:children/resource/resprofile", namespaces=self.ns):
            ...
            for change_pattern in profile.xpath("changepattern"):
                cp_tasks = [element for element in change_pattern.xpath(
                    ".//*") if element.tag in self.task_elements]
                cp_task_labels = [utils.get_label(
                    etree.tostring(task)).lower() for task in cp_tasks]
                ...
                if any(x in ex_tasks or x == utils.get_label(etree.tostring(root)) for x in cp_task_labels):
                    ...
                    root.xpath("cpee1:children/resource/resprofile",
                               namespaces=self.ns).remove(profile)
                    continue

                for task in cp_tasks:

                    attribs = {"type": change_pattern.xpath(
                        "@type"), "direction": change_pattern.xpath("parameters/direction/text()")}
                    task.attrib.update(
                    ...
                    if change_pattern.xpath("@type")[0].lower() in ["insert", "replace"]:
                        ...
                        path = etree.ElementTree(
                            task.xpath("/*")[0]).getpath(task)
                        ...
                        self.lock = True
                        profile.xpath("cpee1:children", namespaces=self.ns)[
                            0].append(task)
...
class ResourceError(Exception):
    ...
    def __init__(self, task, message="{} No valid resource allocation can be found for the given set of available resources"):
        ...
        self.message = message.format(
            utils.get_label(etree.tostring(self.task)))
