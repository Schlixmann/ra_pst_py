...
class TreeGraph():
    ...
    def add_visualization_res(self, element):
        time = element.xpath(f"cpee1:timeslots/cpee1:slot/*", namespaces=self.ns)
        if time: 
    ...
    def add_visualization_resprofile(self, element, measure="cost"):
        ...
        time = element.xpath(f"parent::*/cpee1:timeslots/cpee1:slot/*", namespaces=self.ns)
        if time: 
            time = [f"{value.xpath("name()")}: {value.text}" for value in time]
        else: 
            time = "free"
    ...
    def add_visualization_task(self, element):
        ...
        try:    
            times = {"ready": ready_time[0].text , "start": planned_start[0].text, "end": planned_end[0].text}
        except:
    ...
    def show(self, root, format='png', output_file='graphs/output_graph', view=True, res_option="children"):
        ...
        source.render(filename=f'{out_file}', directory=out_path, cleanup=True, view=view)


if __name__ == "__main__":
    with open("tests/test_output/graphix_test_rapst.xml", "rb") as f:
        #tree = etree.fromstring(f.read())
        TreeGraph().show(f.read(), output_file=f"out_new") 
